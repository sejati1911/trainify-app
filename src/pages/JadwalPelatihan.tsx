...
const handleUpdateJadwal = async (id: number) => {
  ...
  return (
    <div className="p-6 space-y-6 text-slate-800 dark:text-white relative">
      <div className="flex justify-between items-center">
        ...
        <button onClick={fetchAllData} className="p-2.5 bg-white dark:bg-slate-800 hover:bg-slate-700 border border-sky-200 dark:border-slate-700 rounded-lg text-slate-600 dark:text-slate-300 transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {!isSpv ? (
          <form onSubmit={handleCreateJadwal} className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-sky-200 dark:border-slate-700 space-y-4 h-fit">
            <h3 className="font-bold text-sky-400 text-xs uppercase font-mono tracking-wider">Detail Pelatihan</h3>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Nama / Jenis Pelatihan</label>
              <select required value={idPelatihan} onChange={e => setIdPelatihan(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none">
                <option value="">-- Pilih Jenis --</option>
                {types.map(t => <option key={t.id_pelatihan} value={t.id_pelatihan}>{t.nama_pelatihan}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Assign Trainer</label>
              <select required value={idTrainer} onChange={e => setIdTrainer(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none">
                <option value="">-- Pilih Trainer --</option>
                {trainers.map(tr => <option key={tr.id_trainer} value={tr.id_trainer}>{tr.nama_trainer}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Lokasi Pelatihan</label>
              <select required value={idLokasi} onChange={e => setIdLokasi(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none">
                <option value="">-- Pilih Ruang --</option>
                {locations.map(l => <option key={l.id_lokasi} value={l.id_lokasi}>{l.lokasi_pelatihan}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Tanggal Pelatihan</label>
              <input type="date" required value={tanggal} onChange={e => setTanggal(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Waktu Mulai</label>
                <input type="time" required value={waktuMulai} onChange={e => setWaktuMulai(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Waktu Selesai</label>
                <input type="time" required value={waktuSelesai} onChange={e => setWaktuSelesai(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Biaya Pelatihan (Rp)</label>
              <input type="number" required placeholder="Masukkan biaya pelatihan" value={biaya} onChange={e => setBiaya(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-800 dark:text-white focus:outline-none" />
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-sky-200 dark:border-slate-700 h-fit space-y-2">
                  ...
                  <tr key={s.id_jadwal} className="hover:bg-sky-50 dark:hover:bg-slate-500/50 transition-colors">
                    ...
                    <td className="p-3">
                      {editingId === s.id_jadwal && !isSpv ? (
                        <select value={editFields.id_pelatihan || ''} onChange={e => setEditFields({...editFields, id_pelatihan: e.target.value})} className="bg-sky-50 dark:bg-slate-900 text-slate-800 dark:text-white p-1 rounded text-xs w-full">
                          {types.map(t => <option key={t.id_pelatihan} value={t.id_pelatihan}>{t.nama_pelatihan}</option>)}
                        </select>
                      ) : (
                    ...
                    <td className="p-3 text-xs font-mono">
                      {editingId === s.id_jadwal && !isSpv ? (
                        <div className="space-y-1">
                          <input type="date" value={editFields.tanggal_pelatihan || ''} onChange={e => setEditFields({...editFields, tanggal_pelatihan: e.target.value})} className="bg-sky-50 dark:bg-slate-900 text-slate-800 dark:text-white p-0.5 rounded text-xs block w-full" />
                          <div className="flex space-x-1">
                            <input type="time" value={editFields.waktu_mulai || ''} onChange={e => setEditFields({...editFields, waktu_mulai: e.target.value})} className="bg-sky-50 dark:bg-slate-900 text-slate-800 dark:text-white p-0.5 rounded text-[11px] w-1/2" />
                            <input type="time" value={editFields.waktu_selesai || ''} onChange={e => setEditFields({...editFields, waktu_selesai: e.target.value})} className="bg-sky-50 dark:bg-slate-900 text-slate-800 dark:text-white p-0.5 rounded text-[11px] w-1/2" />
                          </div>
                        </div>
                      ) : (
                        <>
                          <p>{s.tanggal_pelatihan}</p>
                          <p className="text-slate-500 dark:text-slate-400 text-[11px]">{s.waktu_mulai?.slice(0,5)}-{s.waktu_selesai?.slice(0,5)} ({s.durasi?.slice(0,5)} jam)</p>
                        </>
                      )}
                    </td>
                    ...
                    <td className="p-3 font-mono text-xs text-emerald-400">
                      {editingId === s.id_jadwal && !isSpv ? (
                        <input type="number" value={editFields.biaya || ''} onChange={e => setEditFields({...editFields, biaya: e.target.value})} className="bg-sky-50 dark:bg-slate-900 text-slate-800 dark:text-white p-1 rounded text-xs w-20" />
                      ) : (
                    ...
                    <td className="p-3 text-center">
                      {editingId === s.id_jadwal && !isSpv ? (
                        <div className="flex justify-center space-x-2">
                          ...
                          <button onClick={() => setEditingId(null)} className="text-slate-500 dark:text-slate-400 hover:text-slate-500 cursor-pointer"><X className="w-4 h-4" /></button>
                        </div>
                      ) : (
                        <div className="flex justify-center space-x-2">
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 border border-sky-200 dark:border-slate-700 rounded-xl w-full max-w-3xl overflow-hidden shadow-2xl">
            <div className="p-4 bg-sky-50 dark:bg-slate-900 border-b border-sky-200 dark:border-slate-700 flex justify-between items-center">
              <h3 className="font-bold text-sm font-mono text-sky-400">{currentJadwalTitle}</h3>
              <button onClick={() => setModalOpen(false)} className="text-slate-500 dark:text-slate-400 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            
            {/* Form Assign Karyawan Baru: Hanya muncul jika BUKAN SPV */}
            {!isSpv ? (
              <form onSubmit={handleAssignPeserta} className="p-4 bg-sky-50/70 dark:bg-slate-900/40 border-b border-sky-200/60 dark:border-slate-700/60 flex items-end space-x-3">
                <div className="flex-1">
                  <label className="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-1 uppercase font-mono">Pilih Karyawan untuk Dimasukkan Kelas</label>
                  <select required value={selectedPernerToAssign} onChange={e => setSelectedPernerToAssign(e.target.value)} className="w-full bg-sky-50 dark:bg-slate-900 border border-sky-200 dark:border-slate-700 rounded-lg p-2 text-xs text-slate-800 dark:text-white focus:outline-none">
              ...
              <div className="p-3 bg-slate-900/20 border-b border-sky-200/60 dark:border-slate-700/40 text-[11px] font-mono text-slate-500 dark:text-slate-400">
            ...
            <div className="p-4 max-h-[350px] overflow-y-auto">
              {loadingPeserta ? (
                <p className="text-xs font-mono text-slate-400 dark:text-slate-500 text-center py-4">Sinkronisasi manifest...</p>
              ) : assignedPeserta.length === 0 ? (
                <p className="text-xs text-slate-500 dark:text-slate-400 text-center py-4">Belum ada karyawan ter-assign di dalam sesi ini.</p>
              ) : (
                <table className="w-full text-left text-xs">
                  <thead className="bg-sky-50 dark:bg-slate-900 font-mono text-slate-500 dark:text-slate-400 uppercase text-[10px]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-sky-200 dark:divide-slate-700 text-slate-700 dark:text-slate-200">
                    {assignedPeserta.map(p => (
                      <tr key={p.perner} className="hover:bg-sky-50 dark:hover:bg-slate-750/30">
                        <td className="p-2.5 font-mono text-sky-400">{p.perner}</td>
                        <td className="p-2.5 font-semibold">{p.nama_peserta}</td>
                        <td className="p-2.5 text-slate-500 dark:text-slate-400">{p.job_position}</td>
